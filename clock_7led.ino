...
#define   BUTTON_SET_MINUTES  6
#define   BUTTON_SET_HOURS    7
...
void setup() {
  ...
  pinMode( BUTTON_SET_MINUTES, INPUT_PULLUP );
  pinMode( BUTTON_SET_HOURS, INPUT_PULLUP );
...
void loop() {
 ...
 if ( btn_release_set_h_tmr.debounce( digitalRead( BUTTON_SET_HOURS ) ) ) {
  ...
  // Dots off for blink
  if ( secondtimer.done() ){
    if ( ! ( btn_min || btn_hrs || temp_display ) ) {
